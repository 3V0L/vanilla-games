...
function getRandomWord() {
  var randomWord = wordList[Math.floor(Math.random() * (wordList.length - 1))].toUpperCase();
  return expectedWord = randomWord.split('');
};

function updateExpectedWord() {
  var expected = [];
  expected.length = expectedWord.length;
  for(var i = 0; i < expectedWord.length; i++) {
    if (expectedWord[i] == ' ') {
      displayLetters[i] = '    ';
    } else if (guessedLetters.includes(expectedWord[i])) {
      displayLetters[i] = expectedWord[i];
    } else {
      displayLetters[i] = '_';
    }
  }
...
function increaseFails() {
  difference = guessedLetters.filter(function(x) { return expectedWord.indexOf(x) < 0 })
  fails = difference.length;
  return "image" + fails;
}

function update() {
  document.getElementById('expected-word').innerHTML = updateExpectedWord();
  document.getElementById('used-letters').innerHTML = guessedLetters.join('  ');
  document.getElementById('game-image').className = increaseFails();

  setTimeout(function () { return document.getElementById('text-box').value = '';},
    800);
  var diff = displayLetters.filter(function(x) { return expectedWord.indexOf(x) < 0 })
  if (fails === 6) {
    alert("GAME OVER!\nTHE WORD WAS " + expectedWord.join(''));
...
function intiateGame() {
  getRandomWord();
  document.getElementById('game-image').className = "image0";
  document.getElementById('expected-word').innerHTML = updateExpectedWord();
  document.getElementById('used-letters').innerHTML = guessedLetters.join('  ');
...
document.getElementById("text-box").addEventListener("keydown", function(event){
  if (event.keyCode >= 65 && event.keyCode <= 90){
    ...
    if (!guessedLetters.includes(String.fromCharCode(keynum))) {
      guessedLetters.push(String.fromCharCode(keynum));
    }
    update();
  } else {
